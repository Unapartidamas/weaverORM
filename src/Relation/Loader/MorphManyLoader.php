...
final readonly class MorphManyLoader
{
    ...
    public function load(array $parents, RelationDefinition $relation, EagerLoadPlan $plan): void
    {
        ...
        $morphTypeColumn = $relation->getMorphType()
            ?? throw new \LogicException(
                sprintf(
                    'MorphMany relation "%s" on "%s" is missing a morphType column name.',
        ...
        $morphIdColumn = $relation->getMorphId()
            ?? throw new \LogicException(
                sprintf(
                    'MorphMany relation "%s" on "%s" is missing a morphId column name.',
                    $relation->getProperty(),
                    $parents[0]::class,
                )
            );
