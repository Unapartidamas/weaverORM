...
final readonly class HasManyThroughLoader
{
    ...
    public function load(array $parents, RelationDefinition $relation, EagerLoadPlan $plan): void
    {
        ...
        $throughEntity = $relation->getThroughEntity()
            ?? throw new \LogicException(
                sprintf(
                    'HasManyThrough relation "%s" on "%s" is missing a throughEntity.',
        ...
        $throughLocalKey = $relation->getThroughLocalKey()
            ?? throw new \LogicException(
                sprintf(
                    'HasManyThrough relation "%s" on "%s" is missing a throughLocalKey.',
                    $relation->getProperty(),
                    $parents[0]::class,
                )
            );
        ...
        $throughForeignKey = $relation->getThroughForeignKey()
            ?? throw new \LogicException(
                sprintf(
                    'HasManyThrough relation "%s" on "%s" is missing a throughForeignKey.',
                    $relation->getProperty(),
                    $parents[0]::class,
                )
            );
